...
export default defineNitroPlugin((nitroApp: NitroApp) => {
    nitroApp.hooks.hook('request', async (event) => {
        ...
        const expiresAt = session.secure?.expiresAt

        // If there's no access token or expiration, the user is not logged in.
        ...
        const isTokenExpired = expiresAt < (Date.now() + 60 * 1000)
        if (!isTokenExpired) {
